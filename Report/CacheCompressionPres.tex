...
\begin{frame}{Implementation of XOR Cache}
      The implementation consists of 2 major components:  
      \begin{enumerate}
          \item Xor Compressor
...
\textit{Note: }Cache size intentionally kept small to improve likelihood of compression events
\end{frame}
...
\begin{itemize}
    \item One of the major challenge was \textbf{gem5 version compatibility}. The older stable version has a known bug where compressed tags are broken which requires to switch to a newer develop branch (v24.1)) but then again, DevBenchmarkBaseline started throwing a ``bad\_function\_call'' error—yet switching back to v23.1 made it run again.
\end{itemize}
...
\begin{frame}{Future Work}
    \begin{itemize}
        \item Run XOR cache on all gem5 benchmarks \& custom built benchmarks.
        \item Record result.
        \item Analysis result and compare with conventional cache compression algorithm.
