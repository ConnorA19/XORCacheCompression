...
\begin{frame}{XOR Cache: XOR Compressor}
            ...
            \If{$N = 0$}
                \State $\text{comp\_lat} \gets 0$
                \State $\text{decomp\_lat} \gets 0$
                \State \Return data
            \EndIf
        
            \State $\text{data.xor}[0] \gets chunks[0]$
        
            \For{$i = 1$ to $N-1$}
                \State $\text{data.xor}[i] \gets chunks[i] \oplus chunks[i-1]$
            \EndFor
        
            \State $\text{data.sizeBits} \gets N \times \text{chunkSizeBits}$
        
            \State $\text{comp\_lat} \gets 
                    \lceil N / \text{compChunksPerCycle} \rceil 
                    + \text{compExtraLatency}$
        
            \State $\text{decomp\_lat} \gets 
                    \lceil N / \text{decompChunksPerCycle} \rceil
                    + \text{decompExtraLatency}$
